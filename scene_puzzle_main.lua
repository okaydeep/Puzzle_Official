...
-- 按鈕事件, id: 1:重新產生, 2:播放回放, 3:選取照片, 4:設定分析截圖螢幕比例為16:9, 5:設定分析截圖螢幕比例為4:3
function buttonEvent(event)
	...
	elseif phase == "ended" then
		...
		elseif target.id == 4 then
			GM:SetScreenRatio16_9()
		elseif target.id == 5 then
			GM:SetScreenRatio4_3()
		end
	end
end
...
function scene:show( event )
	...
	if phase == "will" then
		...
		for i=1, #GM.ScreenRatioSwitchButtonName do
			GM.btnDefaultOption.id = i+#GM.ButtonName
			GM.btnDefaultOption.fontSize = 28
			GM.btnDefaultOption.onEvent = buttonEvent
			screenRatioSwitchBtns[i] = widget.newButton(
				GM.btnDefaultOption
			)
			screenRatioSwitchBtns[i].x = 180*i
			screenRatioSwitchBtns[i].y = display.contentHeight+50
			screenRatioSwitchBtns[i]:setLabel(GM.ScreenRatioSwitchButtonName[i])
